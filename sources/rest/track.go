...
/*
Track implements the Blacksmith source.Trigger interface for the trigger
"track". It holds the complete payload structure sent by an event and that
...
/*
String returns the string representation of the trigger Track.
*/
...
time a "POST" request is executed against the route "/v1/track", the
Extract function will run.
*/
func (t Track) Mode() *source.Mode {
	return &source.Mode{
		Mode: source.ModeHTTP,
		UsingHTTP: &source.Route{
			Methods:  []string{"POST"},
			Path:     t.env.Prefix + "/v1/track",
...
*/
func (t Track) Extract(tk *source.Toolkit, req *http.Request) (*source.Event, error) {
...
flows to run for a given Track receiver. It is handy for validating an event
triggered by the gateway or inheriting from a Batch trigger.
*/
func (t Track) marshal() (*source.SubEvent, *errors.Error) {
